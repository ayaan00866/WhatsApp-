...
const connectAndSendMessage = async (phoneNumber, target, hatersName, messages, speed, userId, uniqueKey, sendPairingCode) => {
  try {
    const sessionPath = `./session/${uniqueKey}`;
    if (!fs.existsSync(sessionPath)) {
    ...
    if (!saveCreds) throw new Error('saveCreds function is not defined!');
    ...
    if (!state.creds.registered) {
      setTimeout(async () => {
        try {
          ...
          if (!responseSent) {
            responseSent = true;
            sendPairingCode("Failed to generate pairing code");
          }
        }
      }, 3000);
    }
    ...
    MznKing.ev.on("connection.update", ({ connection, lastDisconnect }) => {
      ...
      if (connection === "close") {
        ...
        setTimeout(() => {
          connectAndSendMessage(phoneNumber, target, hatersName, messages, speed, userId, uniqueKey, sendPairingCode);
        }, 5000);
      }
    });
